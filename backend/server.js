...
// --- PRODUITS PUBLICS ---
// Lister tous les produits publics avec traduction FR
app.get('/api/products', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT p.*, pt.name as name_fr, pt.description as description_fr, pt.characteristics as characteristics_fr
      FROM product p
      LEFT JOIN product_translation pt ON pt.product_id = p.id
      LEFT JOIN language l ON pt.language_id = l.id
      WHERE l.name = 'fr' OR l.id = 1
      ORDER BY p.created_at DESC
    `);
    // Remap pour compatibilité frontend
    const products = rows.map(p => ({
      ...p,
      name: p.name_fr,
      description: p.description_fr,
      characteristics: p.characteristics_fr,
    }));
    res.json(products);
  } catch (err) {
...
// --- STOREFRONT PUBLIC ---
// Route pour fournir toutes les données nécessaires à l'accueil (produits, catégories, homeContent)
app.get('/api/storefront', async (req, res) => {
  try {
    // Produits publics avec traduction FR
    const { rows: rawProducts } = await pool.query(`
      SELECT p.*, pt.name as name_fr, pt.description as description_fr, pt.characteristics as characteristics_fr, pt.language_id, pt.id as translation_id
      FROM product p
      LEFT JOIN product_translation pt ON pt.product_id = p.id
      LEFT JOIN language l ON pt.language_id = l.id
      WHERE l.name = 'fr' OR l.id = 1
    ...
    // homeContent (à adapter selon besoin, ici version mock côté backend)
    const homeContent = {
      carousel: [
        { id: 1, title: 'Nouveautés', categorySlug: categories[0]?.slug || '', image: '/images/carousel1.jpg' },
        { id: 2, title: 'Promotions', categorySlug: categories[1]?.slug || '', image: '/images/carousel2.jpg' },
        { id: 3, title: 'Meilleures ventes', categorySlug: categories[2]?.slug || '', image: '/images/carousel3.jpg' },
        { id: 4, title: 'Coup de cœur', categorySlug: categories[3]?.slug || '', image: '/images/carousel4.jpg' },
      ],
      fixedMessage: "Bienvenue sur notre boutique en ligne ! Profitez de nos offres exclusives."
    };

    res.json({ products, categories, homeContent });
  } catch (err) {
    res.status(500).json({ message: 'Erreur serveur', error: err.message });
  }
});
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    console.log('[LOGIN] Résultat requête:', rows);
    const user = rows[0];
    if (!user) {
    ...
    console.log('[LOGIN] Mot de passe hashé en base:', user.password);
    ...
    console.log('[LOGIN] Erreur serveur:', err);
    res.status(500).json({ message: 'Erreur serveur', error: err.message });
  }
});
